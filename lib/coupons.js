...
/**
 * Revalida um cupom inteiramente a partir do banco e recalcula o desconto já
 * limitado à margem real do pedido — nunca confia no `discount_amount` que o
 * cliente manda (esse valor só existia pra pré-visualização durante o checkout).
 * Usado em app/api/orders/create e app/api/payments/paypal/create-order, os dois
 * únicos lugares que efetivamente aplicam desconto de cupom num valor cobrado.
 *
 * @param {Object} params
 * @param {Object} params.supabase - cliente Supabase do chamador (server-side)
 * @param {Object|null} params.couponInput - { id, code } vindo do cliente, usado só
 *   como pista de qual cupom buscar — nenhum outro campo dele (discount_amount) é usado.
 * @param {string|null} params.affiliateId
 * @param {number} params.orderSubtotal - subtotal já recalculado no servidor
 * @param {number} params.totalSupplierAmount - margem total do supplier no pedido
 * @param {number} params.totalAffiliateAmount - margem total do afiliado no pedido
 * @returns {Object} `{ coupon, discountAmount }` (discountAmount = 0 se não houver
 *   cupom) ou `{ error }` se o cupom informado não for mais válido.
 */
export async function resolveServerCouponDiscount({ supabase, couponInput, affiliateId, orderSubtotal, totalSupplierAmount, totalAffiliateAmount }) {
  if (!couponInput || (!couponInput.id && !couponInput.code)) {
    return { coupon: null, discountAmount: 0 };
  }

  let query = supabase.from('coupons').select('*').eq('is_active', true);
  query = couponInput.id ? query.eq('id', couponInput.id) : query.eq('code', String(couponInput.code).toUpperCase());
  const { data: coupon, error } = await query.single();

  if (error || !coupon) {
    return { error: 'Cupom não encontrado ou inválido' };
  }

  if (coupon.affiliate_id && coupon.affiliate_id !== affiliateId) {
    return { error: 'Cupom não válido para este afiliado' };
  }

  const now = new Date();
  if (coupon.valid_from && new Date(coupon.valid_from) > now) {
    return { error: 'Cupom ainda não está válido' };
  }
  if (coupon.valid_until && new Date(coupon.valid_until) < now) {
  ...
  if (coupon.min_order_value && orderSubtotal < coupon.min_order_value) {
    return { error: `Valor mínimo do pedido: R$ ${Number(coupon.min_order_value).toFixed(2)}` };
  }

  // Desconto nunca pode passar da margem real disponível no pedido — senão o
  // afiliado (ou o admin, dependendo da origem do cupom) paga pra vender.
  const marginPool = coupon.discount_source === 'affiliate' ? totalAffiliateAmount : totalSupplierAmount;
  const requestedDiscount = orderSubtotal * ((coupon.discount_percentage || 0) / 100);
  const discountAmount = Math.max(0, Math.min(requestedDiscount, Math.max(marginPool, 0)));
