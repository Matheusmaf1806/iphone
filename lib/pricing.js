...
/**
 * Resolve o custo efetivo em BRL a partir do custo cadastrado (que pode estar em
 * dólar) somado ao imposto de importação (%), antes de entrar na cascata de margens.
 *
 * Fluxo: custo (USD ou BRL) -> converte pra BRL pela cotação -> soma imposto (%) ->
 * esse valor é o "costPrice" usado em calculateAllPrices.
 *
 * @param {Object} params
 * @param {number} params.costPrice - Custo cadastrado, na moeda de costCurrency
 * @param {string} [params.costCurrency] - 'BRL' | 'USD' (default 'BRL')
 * @param {number|null} [params.importTaxPercentage] - Override do produto/SKU; se null/undefined, usa defaultImportTaxPercentage
 * @param {number} [params.usdBrlRate] - Cotação USD -> BRL (só é usada quando costCurrency === 'USD')
 * @param {number} [params.defaultImportTaxPercentage] - Imposto padrão global (%)
 * @returns {number} Custo em BRL, já com imposto somado
 */
export function resolveCostPriceBRL({
  costPrice,
  costCurrency = 'BRL',
  importTaxPercentage,
  usdBrlRate = 1,
  ...
  const rate = costCurrency === 'USD' ? (usdBrlRate || 1) : 1;
  const costInBRL = costPrice * rate;
  const taxPercentage = (importTaxPercentage !== null && importTaxPercentage !== undefined)
    ? importTaxPercentage
    : (defaultImportTaxPercentage || 0);
  return costInBRL * (1 + taxPercentage / 100);
}
