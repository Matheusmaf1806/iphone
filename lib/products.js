...
export async function syncProductFromVariants(productId) {
  ...
  const costedActive = active.filter(v => v.is_active !== false && v.cost_price != null && v.cost_price > 0);
  ...
  const updateData = { stock_quantity: totalStock, stock_type: 'limited', has_variants: active.length > 0 };

  if (costedActive.length > 0) {
    // Cada SKU pode ter sua própria moeda/imposto — resolve tudo pra BRL antes de
    // comparar, e o valor gravado no produto já sai convertido (por isso
    // cost_currency='BRL' aqui: é um valor derivado, não um custo cadastrado).
    ...
    const margin = product?.supplier_margin_percentage ? parseFloat(product.supplier_margin_percentage) : 10;
    updateData.cost_price = cheapestCost;
    updateData.cost_currency = 'BRL';
    updateData.import_tax_percentage = null;
    updateData.price = margin < 100 ? cheapestCost / (1 - margin / 100) : cheapestCost;
  }
...
function mapProductData(product) {
  return {
    ...
    costCurrency: product.cost_currency || 'BRL',
    cost_currency: product.cost_currency || 'BRL',
    importTaxPercentage: product.import_tax_percentage != null ? parseFloat(product.import_tax_percentage) : null,
    import_tax_percentage: product.import_tax_percentage != null ? parseFloat(product.import_tax_percentage) : null,
    ...
    hasVariants: !!(product.has_variants || (product.variants || []).some(v => v.is_active !== false)),
    has_variants: !!(product.has_variants || (product.variants || []).some(v => v.is_active !== false)),
