...
export async function getProduct(slug) {
  ...
  try {
    ...
    mapped.accessories = await getCompatibleAccessories(product.id, config);
...
/**
 * Acessórios compatíveis curados pelo admin (ex: Apple Pencil Pro pro iPad Pro) —
 * mostrados como checkbox de "adicionar junto" na página do produto principal.
 * Só retorna acessórios ativos; preço é aplicado depois pela página (mesmo
 * padrão de getRelatedProducts/getCrossSellProducts, que também devolvem o
 * produto "cru" pra applyPrices rodar com o contexto de afiliado certo).
 */
async function getCompatibleAccessories(productId, config) {
  const supabase = createServerClient();
  if (!supabase) return [];
  ...
  return data
    .filter(row => row.accessory && row.accessory.is_active !== false)
    .map(row => ({
      isDefaultChecked: row.is_default_checked,
      product: mapProductData(row.accessory, config),
    }));
}
...
export async function getRelatedProducts(productId, category, limit = 10) {
  ...
  // Config carregada uma única vez — recalcula custo/preço de produtos com
  // variação direto dos SKUs, pra "Mais em <categoria>" nunca mostrar R$ 0,00
  // só porque products.price ficou desatualizado (mesmo fix do admin/home).
  const config = await getPlatformConfig();
  return products.map(product => mapProductData(product, config));
}
...
const CROSS_SELL_MAP = {
  'iphone': ['airpods', 'apple-watch', 'acessorios'],
  'mac': ['acessorios', 'ipad'],
  'ipad': ['acessorios', 'apple-watch'],
  'apple-watch': ['iphone', 'airpods', 'acessorios'],
  'airpods': ['iphone', 'apple-watch', 'acessorios'],
...
export async function getCrossSellProducts(productId, category, limit = 10) {
  ...
  try {
    ...
    // Sem mapa de categorias complementares: qualquer produto de outra categoria serve.
    query = complementary.length > 0
      ? query.in('category', complementary)
      : query.neq('category', category);

    // Busca um pool maior que o limite pedido pra poder diversificar por categoria
    // (ex: iPhone sugerindo AirPods E Apple Watch, não 2 AirPods) em vez de pegar
    // os N primeiros que vierem, que tendem a ser todos da mesma categoria.
    const { data, error } = await query.limit(Math.max(limit * 6, 20));
    if (error || !data) return [];

    const mapped = data.map(product => mapProductData(product, config));

    // No máximo um produto por categoria, na ordem de prioridade do mapa (ou como
    // veio, se não houver mapa) — nunca repete categoria pra preencher o limite
    // (ex: iPhone sugerindo Watch + AirPods, nunca Watch + 2 iPhones só porque não
    // havia acessório cadastrado); se sobrar menos categorias que o limite pedido,
    // o combo simplesmente vem com menos sugestões.
    const priority = complementary.length > 0 ? complementary : [...new Set(mapped.map(p => p.category))];
    const picked = [];
    const usedIds = new Set();
    for (const cat of priority) {
      if (picked.length >= limit) break;
      const found = mapped.find(p => p.category === cat && !usedIds.has(p.id));
      if (found) {
        picked.push(found);
...
export async function updateStock(productId, quantity, movementType, reason = null, createdBy = null) {
  ...
  // Atualizar só se o estoque não mudou desde a leitura acima (compare-and-swap) —
  // sem isso, dois pedidos simultâneos pro último item leem o mesmo valor e os dois
  // decrementos "vencem", vendendo mais do que existia.
  ...
  if (!updated || updated.length === 0) {
    return { success: false, error: 'Estoque alterado concorrentemente, tente novamente' };
  }
...
export async function updateVariantStock(variantId, quantity, movementType, reason = null, createdBy = null) {
  ...
  // Compare-and-swap: só atualiza se o estoque não mudou desde a leitura acima,
  // pra dois pedidos simultâneos pro último SKU não venderem os dois "com sucesso".
  const { data: updated, error: updateError } = await supabase
    .from('product_variants')
    .update({ stock_quantity: newQuantity })
    .eq('id', variantId)
 ...
 */
export function computeVariantRollup(variants, supplierMarginPercentage, config) {
