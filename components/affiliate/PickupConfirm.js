...
export default function PickupConfirm({ session, token }) {
  ...
  const loadOrder = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/affiliate/pickup?token=${encodeURIComponent(token)}`);
  ...
  const handleConfirm = async () => {
    ...
    setConfirmError('');
    try {
      const res = await fetch('/api/affiliate/pickup', {
        method: 'POST',
  ...
  return (
    <AffiliateLayout session={session}>
      <div className="max-w-lg mx-auto">
        <Link href="/afiliado/adm/retiradas" className="text-sm text-gray-600 hover:text-gray-900 inline-flex items-center gap-2 mb-4">
